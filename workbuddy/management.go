...
func managementRegistration() managementRegistrationResponse {
	...
	return managementRegistrationResponse{
		Routes: []managementRoute{
			...
			{Method: http.MethodPost, Path: base + "/accounts/config", Description: "Set per-credential config for one account: priority (scheduling tier), model_aliases, excluded_models (explicit null clears a key)."},
			...
			{Method: http.MethodGet, Path: base + "/models/enterprise", Description: "Enterprise custom model list + refresh status per account."},
		},
...
func handleManagement(raw []byte) ([]byte, error) {
	...
	case req.Method == http.MethodPost && path == base+"/accounts/config":
		return okEnvelope(mgmtJSONResponse(http.StatusOK, handleAccountConfig(req)))
	...
	case req.Method == http.MethodGet && path == base+"/models/enterprise":
		return okEnvelope(mgmtJSONResponse(http.StatusOK, handleEnterpriseModelsStatus()))
	}
